...
@objc public class Utils: NSObject {
    /// 图片裁剪
    @objc public static func cropAndScale(_ image: UIImage, width: Int, height: Int,
                                          orientation: UIDeviceOrientation, mirrored: Bool) -> UIImage? {
        ...
        guard let context = CGContext(data: nil, width: height, height: width,
                                      bitsPerComponent: croppedCgImage.bitsPerComponent,
                                      bytesPerRow: height * croppedCgImage.bitsPerPixel,
                                      space: colorSpace,
                                      bitmapInfo: croppedCgImage.alphaInfo.rawValue) else { return nil }
